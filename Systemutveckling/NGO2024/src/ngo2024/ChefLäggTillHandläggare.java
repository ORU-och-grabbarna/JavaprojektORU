...
public class ChefLäggTillHandläggare extends javax.swing.JFrame {
    ...
    /**
 * Checks if the specified project belongs to the logged-in user.
 *
 * @param projektnamn The name of the project to check.
 * @return {@code true} if the project belongs to the logged-in user, {@code false} otherwise.
 */
private boolean arProjektTillgangligt(String projektnamn) {
    try {
        String sql = "SELECT COUNT(*) FROM projekt " +
                     "WHERE projektnamn = '" + projektnamn + "' " +
                     "AND projektchef IN (SELECT aid FROM anstalld WHERE epost = '" + inloggadAnvandare + "')";
        String resultat = idb.fetchSingle(sql);

        return !resultat.equals("0");
    } catch (InfException e) {
        javax.swing.JOptionPane.showMessageDialog(this, "Fel vid kontroll av projektägare: " + e.getMessage());
        return false;
    }
}
    ...
    javax.swing.JOptionPane.ERROR_MESSAGE);
        ...
        if (!arProjektTillgangligt(projektnamn)) {
            javax.swing.JOptionPane.showMessageDialog(this, 
                "Du kan endast lägga till handläggare till dina egna projekt!", 
                "Åtkomst nekad", 
